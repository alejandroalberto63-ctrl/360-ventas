...
10. **Espera inteligente — el cliente indicó que necesita tiempo**: Si el contexto del lead muestra `espera_indicada.tiene_espera: true`:
    - **Si `proxima_fecha_contacto` es en el futuro** → genera `accion: "esperar"` con `razon_decision: "Lead indicó que necesita tiempo: [descripcion]. Recontactar el [proxima_fecha_contacto]"`. NO envíes ningún mensaje. NO cuentes este silencio como parte del ciclo de 5 seguimientos. El reloj de seguimiento se pausa.
    - **Si `proxima_fecha_contacto` es hoy o ya pasó** → genera acción de seguimiento pero con instrucción PERSONALIZADA al agente: el mensaje debe hacer referencia directa a lo que indicó el cliente (ej: "¿Cómo les fue en la reunión con las madres de familia? ¿pudieron tomar una decisión sobre el 360?"). Este mensaje reemplaza al seguimiento genérico del ciclo. Usa un tono cálido y concreto, no genérico.
    - **Si el cliente indicó espera pero ya han pasado más de 7 días desde `proxima_fecha_contacto`** → retoma el ciclo normal de seguimientos desde donde estaba.
