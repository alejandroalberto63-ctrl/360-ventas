...
function inicioHoyEcuadorMs() {
  ...
  // Construir 00:00 EC (UTC-5) en el día actual de Ecuador
...
(async () => {
  console.log("╔═══════════════════════════════════════════════════════════╗");
  console.log("║         REPORTE GERENCIAL 360 EVENTOS                     ║");
  const fechaHoy = new Date().toLocaleString("es-EC", {
    timeZone: "America/Guayaquil",
    dateStyle: "full",
    timeStyle: "short",
  });
  console.log(`║  ${fechaHoy.padEnd(57)}║`);
  console.log("╚═══════════════════════════════════════════════════════════╝\n");

  const leads = await todosLeadsPipeline();
  console.log(`Total leads pipeline 360: ${leads.length}\n`);
  ...
  for (const e of etapasOrden) {
    ...
    console.log(`  ${e.padEnd(18)} ${String(n).padStart(4)}  ${bar}`);
  }
  const activos = (porEtapa.contacto_inicial || 0) + (porEtapa.seguimiento || 0) + (porEtapa.negociacion || 0);
  console.log(`  ${"─".repeat(50)}`);
  console.log(`  ${"ACTIVOS".padEnd(18)} ${String(activos).padStart(4)}`);
  ...
  console.log("\n━━━ ACTIVIDAD DEL DÍA ━━━");
  console.log(`  Mensajes enviados por el bot:        ${mensajesBotHoy}`);
  console.log(`  Mensajes recibidos de clientes:      ${mensajesClienteHoy}`);
  console.log(`  Videos demo enviados:                ${videosHoy}`);
  console.log(`  Leads cerrados como perdido (hoy):   ${cerradosHoy}`);
  console.log(`  Cierres automáticos (5 msgs / 5d):   ${cerradosAutoHoy}`);
  console.log(`  Leads con cualquier actividad:       ${leadsConActividadHoy}`);

  // ─── 3. POTENCIALES COMPRADORES ─────────────────────────────────────
  console.log("\n━━━ POTENCIALES COMPRADORES (alta intención) ━━━");
  ...
  for (const p of negociacionAlta.slice(0, 10)) {
    console.log(`     #${p.id} | ${(p.nombre || "?").substring(0, 25).padEnd(25)} | nivel=${p.nivel} | precio=$${p.precio || "?"}`);
  }

  // Leads con espera activa (cliente avisará / reunión programada)
  const conEspera = leads
    ...
    .map((l) => {
      ...
      const [_, tipo, fecha] = ultima.match(/espera:(\w+):(\d{4}-\d{2}-\d{2})/) || [];
      return { id: l.id, nombre: l.name, tipo, fecha };
    })
    .filter(Boolean);
  console.log(`\n  ⏳ EN ESPERA (cliente avisará o reunión): ${conEspera.length} leads`);
  for (const p of conEspera.slice(0, 10)) {
    console.log(`     #${p.id} | ${(p.nombre || "?").substring(0, 25).padEnd(25)} | ${p.tipo} | retomar ${p.fecha}`);
  }

  // Leads que respondieron hoy
  ...
  try {
    ...
    console.log(`  Node.js estado:           ${j.estado}`);
    console.log(`  Último ciclo:             ${j.ultimo_ciclo || "ninguno aún"}`);
    console.log(`  Mensajes del último ciclo:${j.ultimo_enviados ?? "—"}`);
    console.log(`  Escalados del último:     ${j.ultimo_escalados ?? "—"}`);

    const r2 = await fetch("https://marketa-360-ventas.hoqkyr.easypanel.host/reporte");
    const j2 = await r2.json();
    if (j2 && j2.duracion_ms) {
      console.log(`  Duración último ciclo:    ${(j2.duracion_ms / 1000).toFixed(1)}s`);
      console.log(`  Contextos LLM vs sintéticos: ${j2.contextos_con_llm}/${j2.contextos_sinteticos}`);
      const ahorro = j2.contextos_sinteticos > 0
        ...
        : 0;
      console.log(`  Ahorro OpenAI por memoria: ${ahorro}%`);
    }
  } catch (e) {
    console.log("  ⚠️ No se pudo consultar Node.js:", e.message);
  }
